...
from .configuration_olmoe2my import Olmoe2MyConfig

from ..olmoe2.modeling_olmoe2 import Olmoe2RMSNorm, Olmoe2Attention, Olmoe2MLP, Olmoe2RotaryEmbedding, load_balancing_loss_func


logger = logging.get_logger(__name__)

_CONFIG_FOR_DOC = "Olmoe2MyConfig"




class Olmoe2MySparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        self.gate = nn.ModuleList([nn.Linear(config.hidden_size, 1, bias=False) for _ in range(self.num_experts)])
...
class Olmoe2MyDecoderLayer(nn.Module):
    def __init__(self, config: Olmoe2MyConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Olmoe2Attention(config=config, layer_idx=layer_idx)

        self.mlp = Olmoe2MySparseMoeBlock(config)
...
@add_start_docstrings(
    "The bare Olmoe2My Model outputting raw hidden-states without any specific head on top.",
    OLMOE2MY_START_DOCSTRING,
)
class Olmoe2MyPreTrainedModel(PreTrainedModel):
    config_class = Olmoe2MyConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["Olmoe2MyDecoderLayer"]
...
class Olmoe2MyModel(Olmoe2MyPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`Olmoe2MyDecoderLayer`]

    Args:
        config: Olmoe2MyConfig
    """

    def __init__(self, config: Olmoe2MyConfig):
        ...
        self.layers = nn.ModuleList(
            [Olmoe2MyDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    @add_start_docstrings_to_model_forward(OLMOE2MY_INPUTS_DOCSTRING)
    # Ignore copy
    def forward(
        self,
...
class Olmoe2MyForCausalLM(Olmoe2MyPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = Olmoe2MyModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        ```python
        >>> from transformers import AutoTokenizer, Olmoe2MyForCausalLM

        >>> model = Olmoe2MyForCausalLM.from_pretrained("allenai/Olmoe2My-1B-7B-0924")
        >>> tokenizer = AutoTokenizer.from_pretrained("allenai/Olmoe2My-1B-7B-0924")
...
__all__ = ["Olmoe2MyForCausalLM", "Olmoe2MyModel", "Olmoe2MyPreTrainedModel"]
